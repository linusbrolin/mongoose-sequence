...
module.exports = function SequenceFactory(connection) {
  ...
  /**
   * Return a pre-update-query hook for this sequence
   * This hook is needed for upsert support with findOneAndUpdate/update/updateOne queries
   *
   * @method     _getPreUpdateHook
   * @return     {Mongoose~Hook} A mongoose hook
   */
   Sequence.prototype._getPreUpdateHook = function () {
    const sequence = this;
    return function (next, done) {
      const query = this;
      const update = this.getUpdate() || this._update.$set || this._update;
      let cb = done;
      if (!sequence._options.parallel_hooks) {
        cb = next;
      }
      ...
      if (!query || !query.model || !update) {
        cb();
        return;
      }
      // disable support for update query with multi: true, for now
      if (query.options && query.options.multi === true) {
        cb();
        return;
      }

      // for query middleware we must do a findOne to see if the doc already exists
      query.model.findOne(query._conditions, (findErr, findObj) => {
        if (findErr) {
          cb(findErr);
          return;
        }
        ...
        sequence._createCounter(update, (createErr, createSeq) => {
          if (createErr) {
            cb(createErr);
            return;
          }
          ...
          if (!_.isNull(createSeq)) {
            query._update['$setOnInsert'][sequence._options.inc_field] = createSeq;
            cb();
          } else {
            sequence._setNextCounter(update, (setError, setSeq) => {
              if (setError) {
                cb(setError);
                return;
              }
              query._update['$setOnInsert'][sequence._options.inc_field] = setSeq;
              cb();
            });
          }
        });
  ...
  Sequence.prototype._setHooks = function () {
    if (this._options.parallel_hooks) {
      ...
      this._schema.pre('findOneAndUpdate', true, this._getPreUpdateHook());
      this._schema.pre('update', true, this._getPreUpdateHook());
      this._schema.pre('updateOne', true, this._getPreUpdateHook());
    } else {
      this._schema.pre('save', this._getPreSaveHook());
      this._schema.pre('findOneAndUpdate', this._getPreUpdateHook());
      this._schema.pre('update', this._getPreUpdateHook());
      this._schema.pre('updateOne', this._getPreUpdateHook());
    }
};
